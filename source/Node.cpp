...
Node::Node(Activation* _activation, int _layerNo = -1, int _nodeNo = -1) :
	activation(_activation), layerNo(_layerNo), nodeNo(_nodeNo)
{
...
// 活性化関数
double ActivationX(double d) {
	return (1.0 / (1.0 + exp(-d)));
}

// 活性化関数を微分
double DeActivationX(double d) {
	return ((1.0 - d) * d);
}
...
void Node::CalcOutput() {
	...
	//outVal = ActivationX(inVal);
	outVal = activation->forword(inVal);
}
// 重みの初期化関数
